...
export type AyTimerBinding = {
	targetId: number;
	lane: AyTimerValueLane | null;
	inlineValue: number;
};

export type AyTimerSegment = {
	startFrame: number;
	endFrame: number;
	retrigger: boolean;
	bindings: AyTimerBinding[];
	rateHz: number;
	intervalRatesHz: number[];
	timerIndex: number;
};

export type AyRegisterImportInput = {
	frames: PsgFrame[];
	frameCount: number;
	frameRateHz: number;
	...
	stMixing: boolean;
	timerSegments: AyTimerSegment[];
	instrumentIdOffset: number;
};

export type AyRegisterImportResult = {
	song: Song;
...
class AyRegisterSongBuilder {
	...
	constructor(private readonly input: AyRegisterImportInput) {
		this.tuningTable = generate12TETTuningTable(this.clockHz(), DEFAULT_A4_HZ);
		this.frames = fitFrameCount(input.frames, input.frameCount);
	}

	build(): AyRegisterImportResult {
		...
		const loopPatternIndex = boundaries.findIndex(
			(boundary) => boundary.startFrame === this.input.loopFrame
		);
	...
	private clockHz(): number {
		return this.input.clockHz > 0 ? this.input.clockHz : DEFAULT_CHIP_FREQUENCY;
	}

	private createSong(): Song {
		const song = new Song();
		song.chipType = 'ay';
		song.chipVariant = this.input.chipVariant;
		song.chipFrequency = this.clockHz();
		song.interruptFrequency = this.input.frameRateHz;
		song.initialSpeed = 1;
		...
		if (this.input.stMixing) {
			settings.stMixing = true;
		}
		if (this.input.stereoLayout) {
			settings.stereoLayout = this.input.stereoLayout;
		}
		return song;
	}

	private resolvePatternBoundaries(): Array<{ startFrame: number; length: number }> {
		const { frameCount, loopFrame } = this.input;
	...
	private resolveTimerEffectsPerFrame(): Array<Array<TimerEffect | null>> {
		const perChannel: Array<Array<TimerEffect | null>> = Array.from(
			{ length: AY_TONE_CHANNELS },
			() => new Array<TimerEffect | null>(this.input.frameCount).fill(null)
		);

		for (const segment of this.input.timerSegments) {
	...
	private toTimerEffect(segment: AyTimerSegment): TimerEffect | null {
	...
	private toSampleBytes(values: number[]): number[] {
		const dacTable =
			this.input.chipVariant === 'YM' ? SAMPLE_DAC_TABLE_YM : SAMPLE_DAC_TABLE_AY;
	...
	private resolveEnvelopeChannel(segment: AyTimerSegment): number {
	...
	private resolveBasePeriod(segment: AyTimerSegment, fineTargetId: number): number {
		const state = this.frameState(segment.startFrame);
...
export function importAyRegisterFrames(input: AyRegisterImportInput): AyRegisterImportResult {
	return new AyRegisterSongBuilder(input).build();
}
