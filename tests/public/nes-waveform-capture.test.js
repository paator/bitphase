...
describe('NesWaveformCapture', () => {
	it('normalizes emulator channel output around zero', () => {
		expect(normalizeNesChannelWaveformSample(0, 0)).toBe(-0.25);
		expect(normalizeNesChannelWaveformSample(0, 15)).toBe(0.25);
		expect(normalizeNesChannelWaveformSample(0, 7)).toBeCloseTo(-0.016666, 5);
	});
	...
	it('prefers emulator channel outputs when available', () => {
		...
		expect(outputs[0]).toBe(0.25);
		expect(outputs[1]).toBe(-0.25);
	});
});
